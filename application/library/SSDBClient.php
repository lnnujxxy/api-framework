...
class SSDBClient extends SSDB {
	...
	private function __construct($host, $port, $timeout_ms=2000) {
		parent::__construct($host, $port, $timeout_ms);
		$this->easy();
	}

	public function getInstance($host, $port, $timeout_ms=2000) {
		$key = join('_', func_get_args());
		if (!self::$instances[$key]) {
			self::$instances[$key] = new SSDBClient($host, $port, $timeout_ms);
		}
